...
class Bird{
  constructor(){
    ...
    this.x = 500; // x pos
    this.y = height/2; // y pos
    this.w = width*0.04; // Width
    this.h = width*0.025; // Height
  ...
  collision(){
    ...
    if(this.y + this.h >= height*0.9){
      this.y = height*0.9 - this.h; // The bird cannot go through the platform
    ...
    for(let i in pipes){
      ...
      if(this.x + this.w >= pipes[i].x && this.x <= pipes[i].x + pipes[i].w && !this.hit){
        if(this.y <= pipes[i].y + pipes[i].h || this.y + this.h >= pipes[i].y + pipes[i].h + pipes[i].space){
          this.lose(); // Call the lose function
        }
  ...
  lose(){
    if(!this.hit){
      if(this.score > parseInt(highscore)) localStorage.setItem("highscore", this.score); // If the user made a highscore then update it in local storage
  ...
  reset(){
    ...
    this.y = height/2; // Reset bird's position
    ...
    for(let i in pipes){
      pipes[i].x = width + 0.5*width*i;
      pipes[i].y = floor(random(-height*0.5, 0));
    }
  }
}
...
function mouseClicked(){
  ...
  if(started && bird.hit){
    ...
    if(mouseX >= width/2 - width*0.3/2 && mouseX <= width/2 &&
       mouseY >= height/2 + width*0.2/2 && mouseY <= height/2 + width*0.2/2 + width*0.1){
